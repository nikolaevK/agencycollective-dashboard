...
### Authorization (RBAC)
12 permission keys enforced at middleware + API level: `dashboard`, `analyst`, `studio`, `jsoneditor`, `adcopy`, `invoice`, `users`, `closers`, `media`, `media_manage`, `admin`, `apitokens`. Stored as `perm_*` columns on `admins` (a new key needs a self-healing column in `ensureCriticalColumns`, e.g. `perm_apitokens`). Super admins (`isSuper`) bypass all checks; `isSuper` cannot be set via API.
...
- **OpenAPI is the source of truth** (`lib/api/openapi.ts`, 142 operations, `x-scope` per op) — consumed by the docs page (`/dashboard/api-docs`, spec-driven), `GET /api/v1/openapi.json` (public), and the MCP tool generator. Dev-only drift guard (`lib/api/driftCheck.ts`) compares spec ↔ route files (paths, methods, AND enforced-vs-advertised scope) inside the openapi.json route; keep them in lockstep.
