...
- **OpenAPI is the source of truth** (`lib/api/openapi.ts`, 152 operations, `x-scope` per op) — consumed by the docs page (`/dashboard/api-docs`, spec-driven), `GET /api/v1/openapi.json` (public), and the MCP tool generator. Dev-only drift guard (`lib/api/driftCheck.ts`) compares spec ↔ route files (paths, methods, AND enforced-vs-advertised scope) inside the openapi.json route; keep them in lockstep.
