...
**Attachments.** The drawer's Attachments section lets the admin pick free-form files (receipts, contract addendums, supporting docs) that ride with the email alongside the invoice PDF. Posted as `attachments` multi-part fields on the same `POST /api/admin/clients/[userId]/invoice/send`; passed to `sendInvoiceEmail` via the new `additionalAttachments` option (distinct from `additionalPdfs`, which is reserved for additional INVOICE PDFs and counts them in the subject). Filenames are sanitised in `emailService` (control chars + `\\` / `/` stripped, capped at 200 chars). **Email-only — attachments are NOT filed in `payout_documents`** and don't appear in the client's Documents tab; they're transient send-time additions. Limits tuned for **Vercel Pro's 4.5 MB request body cap**: 5 files max, 3 MB per file, 3 MB total. A block-list of executable-ish extensions (`exe`, `bat`, `js`, `vbs`, `jar`, etc.) is rejected up-front to avoid silent mail-provider stripping; both sides validate but the server is authoritative. The rebill email body auto-adjusts copy ("along with the supporting files for this cycle") when attachments are present — neutral phrasing so it doesn't imply the customer requested them.
