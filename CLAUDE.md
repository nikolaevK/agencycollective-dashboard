...
- **The ad-account SEND route computes `cycleAnchor` with `billing: null`** (`invoice/send/route.ts`) — anchored on `createdAt` day, ignoring the account's `billing_day`/`last_billed_override`. The directory recomputes WITH those controls, so an account with a custom billing day/override can mismatch and not light up "Invoice sent" on send (registering, which pre-fills the directory's `nextRebillAt`, avoids this).
