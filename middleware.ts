...
const API_PERMISSIONS: { match: (p: string) => boolean; perm: PermKey }[] = [
  ...
  { match: (p) => p.startsWith("/api/admin/deal-contracts"), perm: "closers" },
  { match: (p) => p.startsWith("/api/admin/contract-templates"), perm: "closers" },
...
export const config = {
  matcher: [
    ...
    "/api/admin/deal-contracts/:path*",
    "/api/admin/contract-templates/:path*",
